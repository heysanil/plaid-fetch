/* tslint:disable */
/* eslint-disable */
/**
 * The Plaid API
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.503.5
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { ConsumerReportUserIdentity } from './ConsumerReportUserIdentity';
import {
    ConsumerReportUserIdentityFromJSON,
    ConsumerReportUserIdentityFromJSONTyped,
    ConsumerReportUserIdentityToJSON,
} from './ConsumerReportUserIdentity';

/**
 * UserCreateRequest defines the request schema for `/user/create`
 * @export
 * @interface UserCreateRequest
 */
export interface UserCreateRequest {
    /**
     * Your Plaid API `client_id`. The `client_id` is required and may be provided either in the `PLAID-CLIENT-ID` header or as part of a request body.
     * @type {string}
     * @memberof UserCreateRequest
     */
    client_id?: string;
    /**
     * Your Plaid API `secret`. The `secret` is required and may be provided either in the `PLAID-SECRET` header or as part of a request body.
     * @type {string}
     * @memberof UserCreateRequest
     */
    secret?: string;
    /**
     * A unique ID representing the end user. Maximum of 128 characters. Typically this will be a user ID number from your application. Personally identifiable information, such as an email address or phone number, should not be used in the `client_user_id`.
     * @type {string}
     * @memberof UserCreateRequest
     */
    client_user_id: string;
    /**
     * 
     * @type {ConsumerReportUserIdentity}
     * @memberof UserCreateRequest
     */
    consumer_report_user_identity?: ConsumerReportUserIdentity;
}

/**
 * Check if a given object implements the UserCreateRequest interface.
 */
export function instanceOfUserCreateRequest(value: object): boolean {
    if (!('client_user_id' in value)) return false;
    return true;
}

export function UserCreateRequestFromJSON(json: any): UserCreateRequest {
    return UserCreateRequestFromJSONTyped(json, false);
}

export function UserCreateRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): UserCreateRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'client_id': json['client_id'] == null ? undefined : json['client_id'],
        'secret': json['secret'] == null ? undefined : json['secret'],
        'client_user_id': json['client_user_id'],
        'consumer_report_user_identity': json['consumer_report_user_identity'] == null ? undefined : ConsumerReportUserIdentityFromJSON(json['consumer_report_user_identity']),
    };
}

export function UserCreateRequestToJSON(value?: UserCreateRequest | null): any {
    if (value == null) {
        return value;
    }
    return {
        
        'client_id': value['client_id'],
        'secret': value['secret'],
        'client_user_id': value['client_user_id'],
        'consumer_report_user_identity': ConsumerReportUserIdentityToJSON(value['consumer_report_user_identity']),
    };
}

