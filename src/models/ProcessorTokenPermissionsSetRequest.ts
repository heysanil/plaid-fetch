/* tslint:disable */
/* eslint-disable */
/**
 * The Plaid API
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.503.5
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { Products } from './Products';
import {
    ProductsFromJSON,
    ProductsFromJSONTyped,
    ProductsToJSON,
} from './Products';

/**
 * ProcessorTokenPermissionsSetRequest defines the request schema for `/processor/token/permissions/set`
 * @export
 * @interface ProcessorTokenPermissionsSetRequest
 */
export interface ProcessorTokenPermissionsSetRequest {
    /**
     * Your Plaid API `client_id`. The `client_id` is required and may be provided either in the `PLAID-CLIENT-ID` header or as part of a request body.
     * @type {string}
     * @memberof ProcessorTokenPermissionsSetRequest
     */
    client_id?: string;
    /**
     * Your Plaid API `secret`. The `secret` is required and may be provided either in the `PLAID-SECRET` header or as part of a request body.
     * @type {string}
     * @memberof ProcessorTokenPermissionsSetRequest
     */
    secret?: string;
    /**
     * The processor token obtained from the Plaid integration partner. Processor tokens are in the format: `processor-<environment>-<identifier>`
     * @type {string}
     * @memberof ProcessorTokenPermissionsSetRequest
     */
    processor_token: string;
    /**
     * A list of products the processor token should have access to. An empty list will grant access to all products.
     * @type {Array<Products>}
     * @memberof ProcessorTokenPermissionsSetRequest
     */
    products: Array<Products>;
}

/**
 * Check if a given object implements the ProcessorTokenPermissionsSetRequest interface.
 */
export function instanceOfProcessorTokenPermissionsSetRequest(value: object): boolean {
    if (!('processor_token' in value)) return false;
    if (!('products' in value)) return false;
    return true;
}

export function ProcessorTokenPermissionsSetRequestFromJSON(json: any): ProcessorTokenPermissionsSetRequest {
    return ProcessorTokenPermissionsSetRequestFromJSONTyped(json, false);
}

export function ProcessorTokenPermissionsSetRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProcessorTokenPermissionsSetRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'client_id': json['client_id'] == null ? undefined : json['client_id'],
        'secret': json['secret'] == null ? undefined : json['secret'],
        'processor_token': json['processor_token'],
        'products': ((json['products'] as Array<any>).map(ProductsFromJSON)),
    };
}

export function ProcessorTokenPermissionsSetRequestToJSON(value?: ProcessorTokenPermissionsSetRequest | null): any {
    if (value == null) {
        return value;
    }
    return {
        
        'client_id': value['client_id'],
        'secret': value['secret'],
        'processor_token': value['processor_token'],
        'products': ((value['products'] as Array<any>).map(ProductsToJSON)),
    };
}

